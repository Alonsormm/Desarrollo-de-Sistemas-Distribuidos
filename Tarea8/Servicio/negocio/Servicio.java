...
public class Servicio
{
  ...
  public Response alta(@FormParam("usuario") Usuario usuario) throws Exception
  {
    ...
    try
    {
      PreparedStatement stmt_1 = conexion.prepareStatement("SELECT id_usuario FROM usuarios WHERE id_usuario=?");
      try
      {
        stmt_1.setInt(1,usuario.id_usuario);
      ...
      try
      {
        System.out.println(usuario.fecha_nacimiento);
        ...
        ResultSet rs = stmt_2.getGeneratedKeys();
        while(rs.next()){
          id = rs.getInt(1);
        }
      }
      ...
      {
        PreparedStatement stmt_3 = conexion.prepareStatement("INSERT INTO fotos_usuarios VALUES (0,?,(SELECT id_usuario FROM usuarios WHERE id_usuario=?))");
        try
        {
    ...
    return Response.status(Response.Status.OK).entity(Integer.toString(id)).build();
  }
  ...
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response consulta(@FormParam("id_usuario") int id) throws Exception
  {
    Connection conexion= pool.getConnection();
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          return Response.status(400).entity(j.toJson(new Error("El id_usuario no existe"))).build();
        }
        finally
        {
          rs.close();
